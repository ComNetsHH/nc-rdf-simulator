...
def get_params(run_idx = 0):
    ...
    for run in range(0, 10):
        for num_nodes in [100, 200, 300, 400]:
            for (min_gain, p_L) in [(0, 2.0), (0, 0.32), (10, 0.0)]: #, 
                for q in [2.0]:
...
if __name__ == '__main__':
    ...
    if params != None:
        ...
        if not os.path.isfile(f'./res/v{v}_parsed/summary_{res_file_name}.json'):
            ...
            subprocess.run([
                'python3', 
                'analysis_scripts/parse_detailed_results_v3.py', 
                f'--start_time={start_time}', 
                f'--events_file_name={event_file_name}',
                f'--course_file_name={course_file_name}',
                f'--v={v}',
